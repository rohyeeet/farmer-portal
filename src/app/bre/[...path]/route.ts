...
/**
 * Server-side CSRF proxy for all POST sub-paths under /bre/.
 *
 ...
 *
 * Why this handler sits at /bre/[...path] (not /api/bre/[...path])
 * ----------------------------------------------------------------
 * In development the API base URL is empty, so the portal calls relative
 * URLs like /bre/v1/farmer/bav/. Next.js file-based routing maps this path
 * to src/app/bre/[...path]/route.ts, which is this file.
 * In production NEXT_PUBLIC_API_BASE_URL is the full backend origin so the
 * browser calls the backend directly — this handler is never reached.
 *
...
async function proxy(req: NextRequest, path: string[]): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ detail: 'BRE service is currently unreachable. Please ensure VPN is connected and try again.' }, { status: 502 })
  }
